...
// Use the Twitter actor that Succeeds in your Apify. Option: apidojo~twitter-profile-scraper (profile + tweets).
// To use the failing one (user-only): X_APIFY_ACTOR=V38PZzpEgOfeeWvZY
...
export async function fetchXViaProvider(
  ...
  // Pass limit under multiple keys – different Apify actors use different param names
  const input =
  ...
  // Pinned: Apify may return isPinned/pinned, or profile order often has pinned first
